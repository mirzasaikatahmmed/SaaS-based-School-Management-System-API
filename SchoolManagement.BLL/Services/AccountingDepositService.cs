...
public class AccountingDepositService(
    IUnitOfWork uow,
    ITenantContext tenant,
    ITenantSchemaProvisioner provisioner,
    IStorageService storage,
    IHttpContextAccessor http) : IAccountingDepositService
{
    public async Task<AccountingDepositListResponseDto> GetFilteredAsync(AccountingDepositFilterDto filter, CancellationToken ct = default)
    {
        await Ready(ct);
        Manage();
        var page = filter.Page < 1 ? 1 : filter.Page;
        var size = filter.PageSize is < 1 or > 200 ? 25 : filter.PageSize;
        var (items, total) = await uow.AccountingDeposits.GetFilteredAsync(new AccountingTransactionFilter
        {
        ...
        var data = new List<AccountingDepositResponseDto>();
        foreach (var x in items) data.Add(await MapAsync(x, ct));

        return new AccountingDepositListResponseDto
        {
            Data = data,
            TotalCount = total,
            Page = page,
    ...
    public async Task<AccountingDepositResponseDto> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        await Ready(ct);
        Manage();
        var x = await uow.AccountingDeposits.GetByIdAsync(id, ct) ?? throw new NotFoundException($"Deposit '{id}' not found.");
        return await MapAsync(x, ct);
    }

    public async Task<AccountingDepositResponseDto> CreateAsync(CreateAccountingDepositDto dto, CancellationToken ct = default)
    {
        await Ready(ct);
        Manage();
        if (dto.Amount <= 0)
        ...
        var voucherHead = await ValidateVoucherHeadAsync(dto.VoucherHeadId, VoucherHeadTypes.Income, ct);

        var x = new AccountingDeposit
        {
            Id = Guid.NewGuid(),
            ...
            RefNo = dto.RefNo,
            Amount = dto.Amount,
            DepositDate = dto.DepositDate.HasValue ? DateTime.SpecifyKind(dto.DepositDate.Value.Date, DateTimeKind.Utc) : DateTime.UtcNow.Date,
            PayVia = dto.PayVia,
        ...
        await uow.AccountingDeposits.AddAsync(x, ct);
        ...
        return await MapAsync(await uow.AccountingDeposits.GetByIdAsync(x.Id, ct) ?? x, ct);
    }

    public async Task<AccountingDepositResponseDto> UpdateAsync(Guid id, UpdateAccountingDepositDto dto, CancellationToken ct = default)
    {
        await Ready(ct);
        Manage();
        if (dto.Amount <= 0)
        ...
        var newAccount = await uow.AccountingAccounts.GetByIdAsync(dto.AccountId, ct)
            ?? throw new NotFoundException($"Account '{dto.AccountId}' not found.");
        await ValidateVoucherHeadAsync(dto.VoucherHeadId, VoucherHeadTypes.Income, ct);
        ...
        x.RefNo = dto.RefNo;
        x.Amount = dto.Amount;
        x.DepositDate = DateTime.SpecifyKind(dto.DepositDate.Date, DateTimeKind.Utc);
        x.PayVia = dto.PayVia;
        x.Description = dto.Description;
        x.UpdatedAt = DateTime.UtcNow;
        await uow.AccountingDeposits.UpdateAsync(x, ct);
        await uow.SaveTenantChangesAsync(ct);
        return await MapAsync(await uow.AccountingDeposits.GetByIdAsync(id, ct) ?? x, ct);
    }
    ...
    public async Task DeleteAsync(Guid id, CancellationToken ct = default)
    {
        ...
        await uow.AccountingDeposits.DeleteAsync(x, ct);
        await uow.SaveTenantChangesAsync(ct);
    }

    public async Task<AccountingDepositResponseDto> UploadAttachmentAsync(Guid id, Stream stream, string fileName, string contentType, CancellationToken ct = default)
    {
        await Ready(ct);
        Manage();
        var x = await uow.AccountingDeposits.GetByIdAsync(id, ct) ?? throw new NotFoundException($"Deposit '{id}' not found.");
        ...
        var key = $"{AppConstants.StorageFolders.AccountingDeposits}/{id}/{safeName}";
    ...
    private async Task<AccountingDepositResponseDto> MapAsync(AccountingDeposit x, CancellationToken ct) => new()
    {
        Id = x.Id,
    ...
    private void Manage()
    {
        ...
        if (!r.Contains(AppConstants.Roles.Admin) && !r.Contains(AppConstants.Roles.SuperAdmin) && !r.Contains(AppConstants.Roles.Accountant))
            throw new ForbiddenException("Only Super Admin, School Admin, or Accountant can manage deposits.");
    }
