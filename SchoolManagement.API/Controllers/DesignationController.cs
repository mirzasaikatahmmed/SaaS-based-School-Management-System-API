...
public class DesignationController(IDesignationService service):ControllerBase
{
 ...
 [HttpPatch("{id:guid}"),Authorize(Roles=$"{AppConstants.Roles.SuperAdmin},{AppConstants.Roles.Admin}")] public async Task<IActionResult> Update(Guid id,UpdateDesignationDto dto,CancellationToken ct=default)=>Ok(ApiResponse<DesignationResponseDto>.Ok(await service.UpdateAsync(id,dto,ct),"Designation updated"));
