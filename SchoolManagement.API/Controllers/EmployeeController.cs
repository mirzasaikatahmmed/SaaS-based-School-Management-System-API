...
public class EmployeeController(IEmployeeService service,IEmployeeImportService imports):ControllerBase
{
 ...
 [HttpPut("{id:guid}"),Authorize(Roles=$"{AppConstants.Roles.SuperAdmin},{AppConstants.Roles.Admin}")] public async Task<IActionResult> Update(Guid id,UpdateEmployeeDto dto,CancellationToken ct=default)=>Ok(ApiResponse<EmployeeDetailDto>.Ok(await service.UpdateAsync(id,dto,ct),"Employee updated"));
