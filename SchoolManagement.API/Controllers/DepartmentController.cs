...
public class DepartmentController(IDepartmentService service):ControllerBase
{
 ...
 [HttpPut("{id:guid}"),Authorize(Roles=$"{AppConstants.Roles.SuperAdmin},{AppConstants.Roles.Admin}")] public async Task<IActionResult> Update(Guid id,UpdateDepartmentDto dto,CancellationToken ct=default)=>Ok(ApiResponse<DepartmentResponseDto>.Ok(await service.UpdateAsync(id,dto,ct),"Department updated"));
