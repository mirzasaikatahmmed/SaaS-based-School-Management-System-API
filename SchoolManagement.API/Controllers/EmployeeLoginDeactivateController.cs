...
public class EmployeeLoginDeactivateController(IEmployeeService service):ControllerBase
{
 ...
 [HttpPatch("{id:guid}/activate")] public async Task<IActionResult> Activate(Guid id,CancellationToken ct=default){await service.ActivateLoginAsync(id,ct);return Ok(ApiResponse.Ok("Employee login activated"));}
 [HttpPatch("{id:guid}/deactivate")] public async Task<IActionResult> Deactivate(Guid id,CancellationToken ct=default){await service.DeactivateLoginAsync(id,ct);return Ok(ApiResponse.Ok("Employee login deactivated"));}
